...
def _parse_args() -> Namespace:
    ...
    parser.add_argument(
        "--predict", action="store_true", help="Whether to run prediction"
    )
    ...
    paths.add_argument(
        "--tokenizer_path", type=str, default=None, help="Path to vocab file"
    )
    ...
    trainer.add_argument(
        "--max_steps", type=int, default=-1, help="Max number of steps"
    )
    trainer.add_argument(
        "--use_fast_tokenizer",
        action="store_true",
...
def train(
    ...
    use_wandb: bool,
    python_logger: logging.Logger | None,
    *args,
    **kwargs,
):
    """
    ...
    loggers = [
        CSVLogger(save_dir="logs", name="kodocT5query"),
    ]
    ...
    save_pretrained_path = Path("output") / str(int(time.time()))
    save_pretrained_path.mkdir(parents=True, exist_ok=True)
    python_logger.info(f"Model will be saved to {save_pretrained_path}")
